...
internal let sqlBusyTimeoutConstant = 100.0
...
public final class AWSSubscriptionMetaDataCache {
    ...
    private func createTableIfNeeded() throws {
        try db.run(subscriptionMetadataRecords.create(ifNotExists: true) { table in
            ...
            table.column(operationString, unique: true)
            table.column(operationName)
            table.column(lastSyncDate)
        })
    }
    
    internal func saveRecord(operationString: String, operationName: String, lastSyncDate: Date) throws {
        
        let sqlRecord = subscriptionMetadataRecords.filter(self.operationString == operationString)
        
        let recordCount = try db.scalar(sqlRecord.count)
        
        guard recordCount == 0 else {
            try db.run(sqlRecord.update(self.operationString <- operationString, self.lastSyncDate <- lastSyncDate))
            return
        }
    ...
    internal func getLastSyncTime(operationName: String, operationString: String) throws -> Date? {
        let sqlRecord = subscriptionMetadataRecords.filter(self.operationString == operationString)
