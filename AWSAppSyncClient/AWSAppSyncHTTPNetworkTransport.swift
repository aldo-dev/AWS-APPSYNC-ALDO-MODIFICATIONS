...
public class AWSAppSyncHTTPNetworkTransport: NetworkTransport {
    ...
    public func send<Operation>(operation: Operation, completionHandler: @escaping (GraphQLResponse<Operation>?, Error?) -> Void) -> Cancellable {
        return send(operation: operation, overrideMap: [String:String](), completionHandler: completionHandler)
    }
    ...
    func send<Operation>(operation: Operation, overrideMap: [String: String], completionHandler: @escaping (GraphQLResponse<Operation>?, Error?) -> Void) -> Cancellable {
        ...
        var request = URLRequest(url: url)
        initRequest(request: &request)
        var string = String(data: try! serializationFormat.serialize(value: requestBody(for: operation)), encoding: String.Encoding.utf8) as String!
        
    
        for override in overrideMap {
            string = string!.replacingOccurrences(of: override.key, with: override.value)
        }
